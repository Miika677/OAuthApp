...
def test_create_word_unauthorized(client):
    response = client.post("/api/word", json={"word": "test"})
...
def test_create_word_capitalization(client):
    token = create_token(1)
    client.cookies.set("access_token", token)
    response1 = client.post("/api/word", json={"word": "TEST"})
    response2 = client.post("/api/word", json={"word": "test"})
    response3 = client.post("/api/word", json={"word": "   test   "})
...
def test_check_word(client):
    token = create_token(1)
    client.cookies.set("access_token", token)
    post_response = client.post("/api/word", json={"word": "test"})
...
def test_check_votes(client, db_session):
    ...
    client.post("/api/word", json={"word": "test"})
